...
- **AI 상담사(우하단 ✨ 플로팅)** — 어느 화면에서든 열리는 채팅. 매 요청에 대시보드 전체 요약(부부 정보·진단·자금 배분·로드맵·저축 계좌·결혼 예산/체크리스트·가계부 이번 달·메모)을 `/api/advisor`(로그인 필요)에 보내 상담한다. 모델은 **Claude(`ANTHROPIC_API_KEY` 시크릿, 기본 `claude-opus-5`, `ANTHROPIC_MODEL`로 변경)** 우선, 없으면 Gemini 폴백. 서버는 응답 지연 52초에서 끊고(Hosting 60초), 안전 분류기 거절 시 서버측 fallback(`fallbacks: "default"`)으로 이어간다.
