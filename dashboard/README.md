...
- **부동산** — **요약(진단 연동 KPI + Next Action)** / 진단 / 전략·혜택(+제도 뉴스) / 핫이슈 / 대출계산기 / 청약정보·**실거래 지도(국토부 아파트 매매·전월세 실거래가, 최근 3개월 — 네이버 비공식 API 차단으로 공식 API 전환)**, 카드 클릭 → 주소 지오코딩 지도 이동 / 플랜(진단 연동 브리핑 + 체크형 타임라인)
...
| 아파트 실거래가 | 같은 data.go.kr 계정에서 「아파트 매매 실거래가 상세 자료」+「아파트 전월세 실거래가 자료」 **활용신청만 추가** (키는 동일) | `CHEONGYAK_KEY` 공용 (별도 키면 `MOLIT_KEY`) |
