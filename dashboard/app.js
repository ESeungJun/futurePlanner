...
function Advisor({ user, hh, setHh, theme, setTheme }) {
  ...
  ), /* @__PURE__ */ React.createElement("button", { onClick: () => send(), disabled: busy || !input.trim(), title: "보내기", className: "w-9 h-9 rounded-full bg-[#0A0A0A] text-white flex items-center justify-center shrink-0 disabled:opacity-30" }, /* @__PURE__ */ React.createElement(Icon, { name: "send", size: 15 }))), /* @__PURE__ */ React.createElement("p", { className: "mt-2 text-[10.5px] text-[#B0B0B0] leading-relaxed" }, "대화와 대시보드 요약(소득·자산 포함)이 Anthropic Claude API로 전송돼요(학습에 쓰이지 않는 유료 API). 참고용 상담이며 계약·대출·증여 실행 전 전문가 확인을 권해요.")))));
}
const NAV = [
