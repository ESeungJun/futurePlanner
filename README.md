...
| ✨ AI 상담사 | 우하단 플로팅 채팅 — 대시보드 전체 상태(소득·목표·가계부·결혼 준비·로드맵)를 아는 부부 전담 상담(Claude Sonnet, 키 없으면 Gemini 폴백). 실거래가·청약·LH/SH 공고·뉴스를 직접 조회하고, 하루 한 번 먼저 브리핑, 대화에서 나온 결론을 메모·목표·체크리스트·예산·계좌·가계부에 [적용]으로 반영, 합의한 원칙은 '스킬'로 저장 |
