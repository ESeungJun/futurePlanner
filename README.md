...
| 🏢 부동산 | 소득·자산 진단(프리셋 + 목표 가격 직접 입력), DSR/LTV 대출계산기, 청약 정보 + 일정 캘린더, LH·SH·장기전세·공공주택 공고, 국토부 실거래가 지도(아파트·빌라 매매/전월세 — 카드마다 현행 정책 기준 대출 예상·정책대출 판정, "이 가격을 목표로"), 용어·절차 가이드 |
...
| ✨ AI 상담사 | 우하단 플로팅 채팅 — 대시보드 전체 상태(소득·목표·가계부·결혼 준비·로드맵)를 아는 부부 전담 상담(Claude Sonnet — Gemini 무료 티어 폴백은 `ALLOW_GEMINI_FALLBACK=1`로 켠 경우만, 기본 꺼짐). 실거래가·청약·LH/SH 공고·뉴스를 직접 조회하고, 하루 한 번 먼저 브리핑, 대화에서 나온 결론을 메모·목표·체크리스트·예산·계좌·가계부에 [적용]으로 반영, 합의한 원칙은 '스킬'로 저장 |
